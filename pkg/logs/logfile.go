...
// SetupMessageOnlyLogFile writes normal slog output to stdout and message-only output to a file.
func SetupMessageOnlyLogFile(dataDir string, filePrefix string, minLevel slog.Level) (*os.File, error) {
	...
	logFilePath := filepath.Join(dataDir, fmt.Sprintf("%s-%d.log", filePrefix, time.Now().Unix()))
