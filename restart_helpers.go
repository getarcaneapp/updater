...
func fallbackImageForPlan(plan *restartPlan) string {
	if plan == nil {
		return ""
	}
...
func orderSelfUpdateLast(sorted []deps.ContainerWithDeps, plansByName map[string]*restartPlan, policy LabelPolicy) []deps.ContainerWithDeps {
	if len(sorted) < 2 {
		return sorted
	}
	...
	for _, candidate := range sorted {
		labels := restartCandidateLabels(candidate, plansByName[candidate.Name])
		switch {
...
func restartCandidateLabels(candidate deps.ContainerWithDeps, plan *restartPlan) map[string]string {
